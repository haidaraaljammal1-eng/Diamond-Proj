...
| PATCH | `/contracts/rental/:token/official-contract` | Strict whitelist: personal fields only. `null` clears. Card metadata comes from Stripe-hosted card linking, not the paper. |
| PUT | `/contracts/rental/:token/official-contract/signatures/:slot` | Multipart PNG. Public signing slots: `hirer`, `additional-driver`, `sponsor`. Replaces an earlier capture. |
...
Write rules (public):
- Unknown or system-locked keys → `422`. This covers staff terms, Vehicle OUT/IN damage, vehicle, dates, days, price, card number, CVV and expiry.
- `AWAITING` without `identityReady` → `409 CONTRACT_IDENTITY_NOT_READY`.
- `SIGNED` and later → `409 OFFICIAL_CONTRACT_REVIEW_LOCKED`.
- Vehicle OUT / IN signature slots → `409 OFFICIAL_SIGNATURE_SLOT_UNAVAILABLE` on the public signing link (they belong to Car-Out / Car-In workflows).
...
| Plate Code | `vehicle.plateCode` | `Vehicle.plateCode` when such a field exists; current schema has none → official-contract term `plateCode` fallback (never parsed from plate text) | System / Staff fallback |
...
| Vehicle damage OUT | `vehicleOut.damage` | Structured marks `[{ zone, type }]` on the demo diagrams | Car-Out workflow |
...
| Card number boxes | `card.last4` | **Display-only masked reference** after Stripe-hosted card linking: `•••• •••• •••• 4817`. No PAN, CVV or expiry in Diamond inputs. | Stripe/provider metadata |
...
| Vehicle OUT / IN hirer signature | `signatures.vehicleOutHirer`, `.vehicleInHirer` | Captured by the existing Car-Out / Car-In custody workflows, not contract signing | Custody workflows |
...
- `OfficialContractReviewDraft` (one row per contract) holds:
  - customer overrides
  - safe card last4 copied from Stripe/provider card metadata when available
  - `damageOut` / `damageIn` JSON written by custody workflows, validated against the zone list in `official-contract-interactive.ts`
...
- At signing, `Contract.snapshot.officialContract` freezes the exact legal view, including the assigned vehicle identity. Afterwards GET serves identity, vehicle, terms and safe card reference from the snapshot, while custody events and signature state stay live.
...
- Hirer signature always. The additional driver signature is needed only when additional-driver details are filled, and the sponsor signature only when sponsor details are filled. Vehicle OUT / IN signatures are never needed at this stage.
...
No Customer is created or updated, and there is no Vehicle, payment, Finance or Stripe side effect. Signing does not perform Car-Out; OUT/IN mileage, fuel, actual timestamps, damage and custody signatures remain locked until the existing Car-Out / Car-In workflows. The flow step then becomes `PAYMENT`.
...
PublicRentalScreen (stage "contract")
 → ContractReviewStep        page shell: intro, A4 viewport, Save Changes / Sign Contract / Continue
   → useOfficialContract     hook
     → official-contract.store (Zustand, memory only: edits, damage marks, pending signatures)
...
- **Damage map.** Transparent zones sit over the demo's top, left, right and front/rear drawings, in the same SVG viewBox. Marks are drawn in red on the paper when custody data exists. OUT and IN are read-only on the public signing link; Car-Out / Car-In own editing.
...
- **Card boxes.** 16 paper boxes are display-only. Actual card entry uses Stripe-hosted card/payment UI. Stored safe references render as `••••••••••••1234`; no full PAN, CVV or expiry is entered in Diamond-owned A4 inputs.
- **Save / Sign.**
  - Save sends one PATCH for changed personal fields, then PUT/DELETE for each changed contract-level signature, and reconciles with the backend response.
...
- **Simulation.** Browser-only. Normalized simulated identity also re-evaluates the backend's identity gate for `canEdit` (same rule; the policy lists still come from the backend), so contract signature pads work in demo mode. Card boxes remain display-only and custody remains locked. Save stays local, and Sign runs the simulated accept, which leads to demo payment. Nothing reaches the backend.
