...
- **System admin = full access:** users with any `Role.isSystem` role (today: `system_admin`) bypass per-feature permission denial in the central `hasPermission` / `hasAnyPermission` helpers and receive the full `PERMISSIONS` catalog in `/auth/me`. New catalog keys are included automatically without manually updating an admin permission list. Employee roles remain permission-controlled.
